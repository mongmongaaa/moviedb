...
// 프롭스를 쓰려면 Home 쪽에서    <Route path="/" element={<Home/>}/> 이런게 있어야함 근데 없어서 그냥 여기에 context 불러와서 써야함
// 더보기도 prams를 이용해서 num값으로 클릭했을때바다 페이지 추가
...
function MovieList() {
  ...
  useEffect(() => {
    ...
    const loademovie = async () => {
      const reponse = await axios.get(
        `https://api.themoviedb.org/3/movie/popular?api_key=f89a6c1f22aca3858a4ae7aef10de967&page=${pageNum}`
      );
  ...
  function searchMovies() {
   // 입력 양식에서 검색 버튼을 클릭했을 때 검색 실행
    ...
    if (inputValue){
      const apiUrl = `https://api.themoviedb.org/3/search/movie?query=${inputValue}&api_key=f89a6c1f22aca3858a4ae7aef10de967`;
  ...
  return (
    <>
      ...
      {inputValue ? (
        ...
        <ul>
          {movies.map((movie) => (
            <Link key={movie.id} to={`/Detail/movie2/${movie.id}`}>
              <li>
                <figure>
                  <img
        ...
        <ul>
          {load.map((popularMovies) => (
            <Link key={popularMovies.id} to={`/Detail/movie2/${popularMovies.id}`}>
              <li>
                <figure>
                  <img
